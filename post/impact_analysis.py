...
# These categories are based on Geoscience Australia's NEXIS building
# attribute values. These are likely to be different to other exposure
# datasets, so this must be customised as needed.
...
def main(input_file, res, fmt, output_path=None):
    ...
    plt.savefig(pjoin(output_path, "{0}_SLR_Windspeed_by_age.{1}".format(
        event_num, fmt)), dpi=res)
    plt.close(fig)
    ...
    fig.savefig(pjoin(output_path, f"{event_num}_SLR_by_age.{fmt}"),
    ...
    g.ax.set_xlim((20, 90))
    g.ax.legend(title="Construction era", frameon=True)
    plt.savefig(pjoin(output_path,
                      f"{event_num}_SLC_Windspeed_by_age.{fmt}"),
                dpi=res, bbox_inches="tight")
    plt.close(fig)
    ...
    df.pivot_table(index='LGA_NAME',
        ...
        to_excel(pjoin(output_path, f"{event_num}_damage_state_lga.xlsx"))
    ...
    plt.savefig(pjoin(output_path, f"{event_num}_damage_state_lga.{fmt}"),
                dpi=res, bbox_inches="tight")
    plt.close(fig)
    ...
    tbl = df.pivot_table(index='YEAR_BUILT', columns='Damage state',
                         aggfunc='size', fill_value=0)
    tbl.loc['Total', :] = tbl.sum(axis=0).values
    ...
    fig, ax = plt.subplots(figsize=figsize)
    sns.set_context(context, font_scale=1.)
    ax = sns.countplot(x='Damage state', data=df, order=labels,
                       hue='YEAR_BUILT', hue_order=YEAR_ORDER)
    ax.legend(title="Construction era", frameon=True, loc=0)
    ax.set_ylabel("Number")
    plt.savefig(pjoin(output_path, f"{event_num}_damage_state_by_age.{fmt}"),
                dpi=res, bbox_inches="tight")
    plt.close(fig)
    ...
    ax.set_ylim((0, 1))
    ax.legend(title="Construction era", frameon=True, loc=0)
    plt.savefig(pjoin(output_path, f"{event_num}_SLR_by_damage_state.{fmt}"),
                dpi=res, bbox_inches="tight")
    plt.close(fig)
    ...
    ax.set_ylabel("Number")
    plt.savefig(pjoin(output_path, f"{event_num}_building_age_profile.{fmt}"),
                dpi=res, bbox_inches="tight")
    plt.close(fig)
